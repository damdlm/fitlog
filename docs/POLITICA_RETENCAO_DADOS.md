...
| Registro interno de erros técnicos (`CrashLog`: URL, navegador, mensagem/stack trace, `usuario_id`) | tabela própria | Sem prazo definido hoje — **pendente de definição** | Não implementado ainda | Diagnóstico/correção de falhas técnicas do FitLog — **prazo de retenção pendente de decisão (técnica, não depende de terceiro), recomenda-se um prazo curto (ex.: 90 dias) por não ter finalidade de retenção longa** |
