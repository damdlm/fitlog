...
def novo_treino():
    ...
    return render_template('professor/novo_treino_aluno.html', aluno=current_user)
...
def excluir_exercicio(exercicio_id):
    ...
    if ExercicioService.delete(exercicio_id, user_id=current_user.id):
...
def nova_versao():
    ...
    return render_template('professor/nova_versao_aluno.html', aluno=current_user)
...
def editar_treino_versao(versao_id, treino_codigo):
    ...
    from models import VersaoExercicio, ExercicioCustomizado, Musculo
    ...
    if request.method == 'POST':
        ...
        for ex_id in exercicios_ids:
            # Verificar se já existe como ExercicioCustomizado do usuário
            ...
            if exercicio_existente:
                ...
                # Tentar criar a partir do catálogo usando o hash como ID
                ...
                if exercicio_catalogo:
                    ...
                    if not musculo:
                        musculo = Musculo(nome=musculo_nome.lower(), nome_exibicao=musculo_nome)
        ...
        # Remover exercícios antigos e readicionar
        ...
        try:
            ...
            if exercicios_criados > 0:
                flash(f'Treino atualizado! {exercicios_criados} novo(s) exercício(s) do catálogo adicionados.', 'success')
            else:
                flash(f'Treino {treino_codigo} atualizado com sucesso!', 'success')
            return redirect(url_for('aluno.ver_versao', versao_id=versao_id))
        except Exception as e:
            db.session.rollback()
            logger.error(f"Erro ao atualizar treino: {str(e)}")
            flash(f'Erro ao atualizar treino: {str(e)}', 'danger')

    # GET - mostrar formulário
    exercicios_banco = ExercicioCustomizado.query.filter_by(usuario_id=current_user.id).all()


    # Buscar exercícios do catálogo
    catalogo_exercicios = CatalogoService.get_todos_exercicios(limite=500)
    
    # Combinar em uma lista para exibição
    exercicios_display = []
    
    # Adicionar exercícios do banco primeiro
    ...
    # Adicionar exercícios do catálogo (com IDs hash)
    for ex in catalogo_exercicios:
        # Verificar se já não existe no banco
        existe_no_banco = False
        for ex_banco in exercicios_banco:
            if ex_banco.nome.lower() == ex['nome'].lower():
