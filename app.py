...
def get_ultimas_series(exercicio_id, limite=1):
    ...
    for r in reversed(registros):
        if r["exercicio_id"] == exercicio_id:
            # Garantir que tem o campo series
...
def buscar_musculo_no_catalogo(nome_exercicio):
    ...
    try:
        ...
        # 1. Tenta correspondência exata (ignorando maiúsculas/minúsculas)
        for ex in catalogo:
        ...
        # 2. Se não encontrar, tenta correspondência parcial (o nome do catálogo CONTÉM o nome buscado)
        for ex in catalogo:
        ...
        # 3. Se ainda não encontrar, tenta o inverso (o nome buscado CONTÉM o nome do catálogo)
        for ex in catalogo:
            nome_catalogo = ex.get('name', '').lower()
...
def salvar_exercicio():
    ...
    # --- LÓGICA CORRIGIDA PARA DEFINIR O MÚSCULO ---
    musculo_final = None

    # 1. Verifica se o usuário selecionou um músculo no select
    if musculo and musculo.strip() != "":
        musculo_final = musculo
        print(f"Músculo selecionado manualmente: {musculo_final}")
    
    # 2. Se não selecionou, tenta buscar no catálogo
    if musculo_final is None:
        ...
        if musculo_encontrado:
            ...
            print(f"Músculo encontrado no catálogo: {musculo_final}")
    
    # 3. Se ainda não tem músculo (não selecionou e não encontrou), usa "Outros" como padrão
    if musculo_final is None:
        musculo_final = "Outros"
        flash("Músculo não encontrado no catálogo. Usando 'Outros'.", "warning")
        print("Usando músculo padrão: Outros")
